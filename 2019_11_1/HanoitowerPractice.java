...
public class HanoitowerPractice {
    //（1）把n-1个盘子由A 移到 B；
    //（2）把第n个盘子由 A移到 C；
    //（3）把n-1个盘子由B 移到 C；
    ...
    public static void main(String[] args){
        ...
        HanoitowerPractice.HanoiTower(disks, A, B, C);
        System.out.println(">>移动了" + m + "次，把A上的圆盘都移动到了C上");
        imput.close();
    }
    //实现移动的函数
    public static void move(int disks,char N,char M){   //把1编号为n的圆盘从N 移到M上
        System.out.println("第" + (++m) +" 次移动 : " +" 把 "+ disks+" 号圆盘从 " + N +" ->移到->  " + M);
    }
    //递归实现汉诺塔的函数
    public static void HanoiTower(int n,char A,char B,char C){   //把编号为n的圆盘借助B从A移动到 C上
        if (n==1){  //圆盘只有一个时，只需将其从A塔移到C塔
            HanoitowerPractice.move(1,A,C);
        }else {
            HanoiTower(n-1,A,C,B);  //递归，把A塔上编号1~n-1的圆盘移到B上，以C为辅助塔
            HanoitowerPractice.move(n,A,C);   //把A塔上编号为n的圆盘移到C上
